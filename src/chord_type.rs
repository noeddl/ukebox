...
impl ChordType {
    ...
    pub fn symbols(self) -> impl Iterator<Item = &'static str> + 'static {
        ...
        let symbols = match self {
            Major => vec![""],
            MajorSeventh => vec!["maj7"],
            MajorNinth => vec!["maj9"],
            MajorEleventh => vec!["maj11"],
            MajorThirteenth => vec!["maj13"],
            MajorSixth => vec!["6"],
            SixthNinth => vec!["6/9"],
            DominantSeventh => vec!["7"],
            DominantNinth => vec!["9"],
            DominantEleventh => vec!["11"],
            DominantThirteenth => vec!["13"],
            DominantSeventhFlatNinth => vec!["7b9"],
            DominantSeventhSharpNinth => vec!["7#9"],
            DominantSeventhFlatFifth => vec!["7b5"],
            //DominantSeventhSharpFifth => "7#5",
            SuspendedFourth => vec!["sus4"],
            SuspendedSecond => vec!["sus2"],
            DominantSeventhSuspendedFourth => vec!["7sus4"],
            DominantSeventhSuspendedSecond => vec!["7sus2"],
            Minor => vec!["m"],
            MinorSeventh => vec!["m7"],
            MinorMajorSeventh => vec!["mMaj7"],
            MinorSixth => vec!["m6"],
            MinorNinth => vec!["m9"],
            MinorEleventh => vec!["m11"],
            MinorThirteenth => vec!["m13"],
            Diminished => vec!["dim"],
            DiminishedSeventh => vec!["dim7"],
            HalfDiminishedSeventh => vec!["m7b5"],
            Fifth => vec!["5"],
            Augmented => vec!["aug"],
            AugmentedSeventh => vec!["aug7", "7#5"],
            AugmentedMajorSeventh => vec!["augMaj7"],
